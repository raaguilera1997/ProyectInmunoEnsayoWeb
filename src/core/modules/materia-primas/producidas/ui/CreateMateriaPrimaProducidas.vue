<template>
  <q-page class="q-pa-md bg-grey-1">
    <q-form @submit="Save" autocomplete="off">
      <div class="text-subtitle1 text-bold">Crear Materia Prima Producidas</div>
      <q-separator/>
      <q-card flat class=" bg-grey-1">
        <q-card-section>
          <div class="row">
            <div class="col-md-4 col-lg-6 q-px-sm q-py-sm">
              <q-select  @update:model-value="UpdateNom"  dense outlined v-model="nomencladorMateriaPrimaProducidaId" option-label="name" option-value="name" :options="optionsNomenclator" label="Nombre *" :rules="[val => !!val || 'El campo es requerido']" />
            </div>
            <div class="col-md-4 col-lg-6 q-px-sm q-py-sm">
  ...
  export default {
    ...
    methods:{
      ...
      Save() {
        ...
        let url = 'materiaPrimasProducidas'
        ...
        let object = {
          nomencladorMateriaPrimaProducidaId: this.nomencladorMateriaPrimaProducidaId.id,
           codigo:this.codigo,
           lote:this.lote,
           sizeLote:this.sizeLote.toString(),
           ...
           API_REST_POST_REQUEST({ endpoint: url, payload: object }).then(resp => {
            if (resp.status == 200) {
              notify({
                content: 'Materia Prima Producida creada correctamente',
                type: 'positive'
              })
              this.$q.loading.hide()
