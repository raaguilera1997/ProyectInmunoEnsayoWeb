<template>
  <q-page class="q-pa-md bg-grey-1">
    <q-form @submit="Save" autocomplete="off">
      <div class="text-subtitle1 text-bold">Editar Solución Aditiva</div>
      <q-separator/>
      <q-card flat class=" bg-grey-1">
        <q-card-section>
          <div class="row">
            <div class="col-md-4 col-lg-6 q-px-sm q-py-sm">
              <q-select @update:model-value="UpdateNom" dense outlined v-model="nomencladorSolucionAditiva" option-label="name" option-value="name"
...
<script>
  ...
  export default {
    name: 'EditarPSolucionesAditivas',
    data() {
      return {
    ...
    methods: {
      ...
      getInfo() {
        let url = `solucionAditiva/${this.$route.params.id}`;
        this.$q.loading.show({
        ...
        API_REST_GET_REQUEST({ endpoint: url }).then(resp => {
          this.nomencladorSolucionAditiva = resp.data.nomencladorSolucionAditiva;
      ...
      Save() {
        ...
        let object = {
          nomencladorSolucionAditivaId: this.nomencladorSolucionAditiva.id,
          codigo: this.codigo,
          lote: this.lote,
        ...
        API_REST_PUT_REQUEST({ endpoint: url, payload: object }).then(resp => {
          if (resp.status == 200) {
            notify({
              content: 'Solución aditiva editada correctamente',
              type: 'positive'
            });
            this.$q.loading.hide();
